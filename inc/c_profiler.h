...
#define CPROF_DECLARE_LOG(func_name)                                         \
  ...
  static int __cprof_registered_##func_name = 0;  // Used for registration tracking
...
#define CPROF_REGISTER_LOG(func_name)                                    \
  do {                                                                   \
    if (__cprof_registered_##func_name == 0) {                           \
      ...
      func_name##_log.next = __cprof_log_root; /* New node points to old root */ \
...
// Core timing macro:
// It uses a C block ({...}) to ensure timing variables are only valid within that block.
// func_name: Unique name of the function (usually the function name itself)
// code: The original code to be executed by the function
#define CPROF_SCOPE(func_name, code)                                          \
  do {                                                                        \
    /* 1. Ensure registration (now only responsible for registration, not variable definition) */ \
...
// These must be defined as static inline to ensure they are only compiled within each C
// file, and the compiler will try to inline them to reduce function call overhead.
...
static inline void CPROF_dump_to_file(const char* filename) {
  ...
  while (current != NULL) {
    ...
    // Convert nanoseconds (ns) to microseconds (us) for output (because your TimeDiff outputs nanoseconds)
...
static inline void CPROF_cleanup(void) {
  ...
  while (current != NULL) {
    ...
    current->size = 0;
    current->capacity = 0;
    current = current->next;  // Only need to move pointer here, because TimeLog
